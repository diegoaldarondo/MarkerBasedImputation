...
#SBATCH --export=ALL
#SBATCH -o logs/Job.multiGpuTrain.%N.%j.out    # STDOUT
#SBATCH -e logs/Job.multiGpuTrain.%N.%j.err    # STDERR
...
do
   srun -l --gres=gpu:1 -n1 -N1 --mem=40000 cluster/py.sh $FUNC ${DATAPATH[count]} --base-output-path=${BASEOUTPUTPATH[count]} --epochs=30 --stride=5 &
   count=$(( $count + 1 ))
done
