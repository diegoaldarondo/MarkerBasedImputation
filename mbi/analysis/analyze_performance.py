...
def analyze_marker_predictions(model, total, totalR, marker_means, marker_stds, viz_directory):
    """
...
def analyze_performance(model_base_path, data_path, *,
    ...
    output_length = model_info['output_length'][:]
    n_markers = model_info['n_markers'][:]
    ...
    [input_ids,target_ids] = get_ids(bad_frames,input_length,output_length + max_gap_length,True,True)

    # Get the data corresponding to the indices
    X = markers[input_ids[:,:],:]
    Y = markers[target_ids[:,:max_gap_length],:]
    XR = markers[target_ids[:,:(max_gap_length-1):-1],:]
    YR = Y[:,::-1,:]
    ...
    analyze_marker_predictions(model, total, totalR, marker_means, marker_stds, viz_directory)
