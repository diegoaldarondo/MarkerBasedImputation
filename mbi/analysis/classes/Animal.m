...
%             end
        ...
        function h = bradyMovie(obj,markerset,tiles,varargin)
            ...
            for i = 1:nAnimations   
                ...
                switch markerset{i}
                    case 'aligned'
                        x{i} = obj.alignedMarkers(bradyInds{i},:);
                        titles{i} = 'Aligned unimputed';
                    case 'imputed'
                        x{i} = obj.imputedMarkers(bradyInds{i},:);
                        titles{i} = 'Aligned imputed';
                    case 'global'
                        x{i} = obj.markers(bradyInds{i},:);
                        titles{i} = 'Global';
                    otherwise
                        error(['Improper markerset. Must be aligned, ' ...
                            'imputed, or global.']);
            ...
            % Link the axes of the MarkerAnimator objects
            obj.hlinkBrady = linkprop(h{1}.Parent.Children(isMM(end:-1:1)),...
                {'CameraPosition','CameraUpVector'});
